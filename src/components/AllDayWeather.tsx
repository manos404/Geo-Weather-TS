...
`;
const Time = styled.p<StyledComponentsProps>`
  grid-column: 1/2;
  grid-row: 1/2;
  color: ${(props) =>
    props.$isDayTime
...
`;
const H3 = styled.h3<StyledComponentsProps>`
  color: ${(props) =>
...
interface AllDayWeatherProps {
  list: List[];
  isDayTime: boolean;
}
const AllDayWeather: React.FC<AllDayWeatherProps> = ({ list, isDayTime }) => {
  const isWindowLessThan700 = useMediaQuery("(max-width:699px)");

  const getIcon = (weatherId: number, isDayTime: boolean) => {
  ...
  function createH1ElementsFromArray(list: List[]) {
    ...
    return data.map((item, index: number) => {
      ...
      return (
        ...
        >
          <Time
            $isDayTime={isDayTime}
            $isCloudy={list[0].weather[0].main === "Clouds"}
          >
            {time}
          </Time>
