...
const H2 = styled.h2<StyledComponentsProps>`
...
const P = styled.p<StyledComponentsProps>`
  color: ${(props) =>
    props.$isDayTime
      ? props.$isCloudy
        ? "var(--colour-gray)"
      ...
      : "var(--colour-black)"};
`;
const Temp = styled.h1<StyledComponentsProps>`
  color: ${(props) =>
    !props.$isDayTime ? "var(--colour-white)" : "var(--colour-black)"};
`;

const TempFeel = styled.h2<StyledComponentsProps>`
  color: ${(props) =>
    !props.$isDayTime ? "var(--colour-white)" : "var(--colour-black)"};
`;
...
const CurrentWeather: React.FC<CurrentWeatherProps> = ({
  ...
  const getAnimatedIcon = (weatherId: number, isDayTime: boolean) => {
    if (weatherId > 200 && weatherId <= 232) {
      ...
      return isDayTime ? clearDayAnim : clearNightAnim;
  ...
  return (
    <SectionCurrentWeather>
      <WeatherOverview>
        <H2 $isDayTime={isDayTime}> {weatherData.weather[0].description}</H2>
